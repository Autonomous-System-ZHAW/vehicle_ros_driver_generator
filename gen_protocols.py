...
def gen_control_header(car_type, protocol, output_root_dir):
    ...
    with open(control_header_file, 'w') as h_fp:
        ...
        for vars in protocol["vars"]:
            ...
            signal_name = vars["name"].lower()
            signalname_list += "".join("%s %s, " % (signal_type, signal_name))
...
"""
    ...
    for i in range(0, len(byte_info)):
        ...
        impl = impl + fmt % (mask_bit, i, i, info["start_bit"],
                             info["len"], info["byte"], i)
        shift_bit = info["len"]
    return impl
...
def gen_control_value_func_impl(classname, var, protocol):
    ...
    fmt = """// config detail: %(config)s
void %(classname)s::set_p_%(var_name)s(%(var_type)s %(var_name)s) {"""
    fmt_val = {}
    fmt_val["classname"] = classname
...
def gen_control_cpp(car_type, protocol, output_root_dir):
    ...
    with open(control_cpp_file, 'w') as fp:
        ...
        for var in protocol["vars"]:
            ...
            set_private_var = "  set_p_%s(%s);" % (var["name"].lower(),
                                                    var["name"].lower())

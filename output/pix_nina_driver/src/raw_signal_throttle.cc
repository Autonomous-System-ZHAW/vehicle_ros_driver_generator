...
void RawSignalThrottle::update_bytes(uint8_t bytes_data[8])
{
  for(uint i=0;i<8;i++)
  {
...
void RawSignalThrottle::Parse() {
  uint32_signal_throttle_b_ = uint32signalthrottleb();
  uint32_signal_throttle_a_ = uint32signalthrottlea();
}
...
int RawSignalThrottle::uint32signalthrottleb() {
  Byte t0(*(bytes + 7));
...
int RawSignalThrottle::uint32signalthrottlea() {
  Byte t0(*(bytes + 3));
  int32_t x = t0.get_byte(0, 8);

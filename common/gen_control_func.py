...
def msg_signal_list(protocol):
    ...
    result = "%s_msg."%(message_name) + prefix.join(signal_list)[:-2]
...
def msg_received_timestamp_code(protocol, car_type):
  ...
  if({message_name}_delta_time_ms > param_.timeout_threshold_ms.pix_interface_driver_eorro){{
    {message_name}_delay_.count_eorro++;
  }}else if ({message_name}_delta_time_ms > param_.timeout_threshold_ms.pix_interface_driver_warn){{
    {message_name}_delay_.count_waring++;
    ...
    func = """\nstatic void %s_callback(const pix_%s_driver_msgs::%s &msg)
{
    ...
    can_{can_name}.dlc = 8;
    {name}_parse.UpdateData(
        {name_msg_signal_list}
    );
    can_{can_name}.id = {name}_parse.ID;
    can_{can_name}.is_extended= false;
    uint8_t *A;
    A = {name}_parse.get_data();
